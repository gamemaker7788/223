// export-app.js - 导出用 JavaScript 功能模板
const ExportApp = {
    getJS(weekNum = 1) {
        return `
/* ================= 常量 ================= */
...
function showProfile(){
  ...
  const username = u && u.username ? u.username : '未登录';
  const userId = u && u.id ? u.id : '';
  
  document.getElementById('mainContent').innerHTML = '<h1>个人设置</h1>' +
    '<section class="card">' +
      '<h3>账号信息</h3>' +
...
function showChat(){
  closeSide();
  document.getElementById('mainContent').innerHTML='<h1>聊天室</h1>' +
    '<section class="card">' +
      '<h1>消息列表</h1>' +
...
/* ========== 登录页 ========== */
function showLogin(){
  document.getElementById('mainContent').innerHTML='<h1>223班聊天室</h1>' +
    '<section class="card">' +
      '<h3>登录 / 注册</h3>' +
